...
export const skipNotifications = async () => {
	await expect(
		element(by.id(testIDs.NOTIFICATON.SKIP_BUTTON))
	).toBeVisible();
...
export const setBirthDate = async (day, month, year) => {
	...
	await element(by.id(testIDs.QUESTION.PICKER_DAY)).setColumnToValue(
		0,
		day
	);
...
export const deleteAccount = async () => {
	...
	await element(
		by.id(testIDs.DELETE_DATA.DELETE_DATA_MODAL_BUTTON)
	).tap();
